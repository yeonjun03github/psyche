...
const SECTION_GUIDANCE: Record<string, string> = {
  ...
  currentMentalHealthStatus:
    '지금 이 순간의 상태에 대한 스냅샷(검사에서 직접 확인되는 사실)만 다룬다. "왜" 이렇게 됐는지, 최근 심해졌는지/오래됐는지는 여기서 다루지 않는다 — 시작 시점이나 변화 여부는 수검자가 직접 보고하지 않은 이상 알 수 없다(그 가능성만 possibleCausalHypotheses의 몫).',
  primaryConcern:
    '여러 문제 중 지금 가장 먼저 봐야 할 단 하나를 선택하고 그 이유를 설명한다. currentMentalHealthStatus의 상태 설명을 반복하지 말고, "왜 이것이 다른 것보다 더 시급한가"에만 집중한다. 시급함의 근거도 검사 결과 범위 안에서만 제시하고 원인은 다루지 않는다.',
  primaryStrength: '약점의 반대가 아니라, 지금 이 사람이 실제로 기댈 수 있는 자원 하나를 짚는다.',
  crossTestCorrelations:
    '겉보기에 무관해 보이는 검사 결과 두 개 이상이 어떻게 맞물리는지 발견하는 섹션. 이미 언급한 개별 결과를 재서술하지 말고, "관계" 자체가 새로운 정보여야 한다. 상관(함께 나타남)과 인과(원인→결과)를 혼동하지 말 것 — "A와 B가 함께 나타난다"까지만 쓰고 "A 때문에 B가 나타났다"로 확장하지 않는다.',
  possibleCausalHypotheses:
    '지금 상태가 왜 시작됐을 가능성이 있는지(기원)만 다루되, 전부 확인되지 않은 가설임을 전제한다. 사용자 메모(수검자가 보고한 최근 상황)를 그 자체로 원인처럼 서술하지 말 것 — "~와 관련되어 있을 가능성을 생각해볼 수 있습니다" 수준으로만 쓰고, 시작 시점을 모르면(대부분 그렇다) 최소 하나의 대안적 설명이나 "현재 자료만으로는 판단할 수 없습니다"라는 문장을 반드시 포함한다. maintainingFactors(왜 계속되는지), aggravatingFactors(뭐가 더 나쁘게 만들 수 있는지)와는 다른 시점의 질문임을 명심한다.',
  maintainingFactors:
    '원인이 무엇이었든, 지금 이 상태를 계속 지속시키는 요인일 "가능성"만 다룬다. 성격 특성(예: 높은 성실성) 하나만으로 실제 행동 패턴(예: "충분히 쉬지 못하고 있다")을 단정하지 말고, 반드시 "~하고 있을 가능성이 있습니다" 같은 가설 어조로 쓴다. possibleCausalHypotheses의 원인 설명을 반복하지 않는다.',
  aggravatingFactors:
    '아직 벌어지지 않았거나 더 커질 수 있는 잠재적 위험 요인일 "가능성"만 다룬다. 이미 벌어진 일처럼 단정하지 말고, 이미 작동 중인 maintainingFactors와 겹치지 않도록 "앞으로 나빠질 수 있는 조건"에 집중한다.',
...
- 검사 결과 두 개 이상이 같은 방향을 가리키는 것(상관)과, 하나가 다른 하나의 원인이라는 것
  ...
  확장하지 마십시오.
- 원인·유지·악화를 다루는 섹션(possibleCausalHypotheses, maintainingFactors,
  aggravatingFactors)을 쓸 때는 가능하면 대안적 설명이 있음을 함께 언급하십시오 — 예: "직장
  생활과 관련되어 있을 가능성을 생각해볼 수 있지만, 그 상태가 이전부터 있었는지, 다른 요인이
  있는지는 현재 자료로 구분할 수 없습니다." 사용자가 제공하지 않은 특정 원인(예: "수면 문제일
  것이다")을 실제 원인처럼 추정하지 마십시오 — "수면 패턴에 변화가 있었는지 추가 확인이
  필요합니다" 정도로만 쓰십시오.
- 알 수 없는 정보를 임의로 채우지 마십시오. 시작 시점, 이전 상태와의 비교, 사건 전후 변화처럼
  현재 자료에 없는 정보는 필요한 곳에서 "현재 자료만으로는 알 수 없습니다"라고 명시하십시오.
  "알 수 없음"은 실패한 결과가 아니라 현재 데이터가 허용하는 정확한 결론입니다. 정확하지 않은
  확신보다 제한된 정보에 대한 솔직한 불확실성이 항상 우선합니다.
...
3. 원인·유지·악화 요인을 다루는 항목(possibleCausalHypotheses, maintainingFactors,
   ...
   가설 표현만 사용하고, "때문이다"처럼 단정적인 인과 표현은 사용하지 마십시오(위 원칙 참고).
...
10. 사용자가 남긴 참고 메모(사용자 메모)가 주어지면, 이는 검증된 사실이 아니라 "수검자가 보고한
    상황"입니다. 리포트 서두(overallSummary 또는 currentMentalHealthStatus)에서 "수검자는
    ~라고 보고했습니다"처럼 출처를 밝혀 그 맥락을 자연스럽게 언급하십시오. 그 상황을 검사
    결과의 원인으로 단정하지 말고, 위 "사실·보고·해석·가설" 원칙에 따라 관련 가능성 정도로만
    연결하십시오.
