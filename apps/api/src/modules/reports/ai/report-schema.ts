...
export const reportSectionsSchema = z.object({
  ...
  confirmedStatus: z.string().min(1),
  confirmedStrength: z.string().min(1),
  crossTestPatterns: z.string().min(1),
  // 참고 메모가 없으면(reportedSituation과 함께) null이어야 한다 — processor에서 방어적으로도 강제한다.
  reportedSituation: z.string().nullable(),
  possibleRelevance: z.string().nullable(),
  unknownFromCurrentData: z.string().min(1),
  suggestedFollowUps: z.string().min(1),
  selfCareDirections: z.string().min(1),
  metricsToTrack: z.string().min(1),
  retestGuidance: z.string().min(1),
